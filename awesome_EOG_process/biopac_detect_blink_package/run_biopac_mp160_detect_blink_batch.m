...
%==========================================================================
    ...
    if nargin < 2 || isempty(output_dir)
        output_dir = fullfile(input_dir, 'detect_blink_output');
    end
    ...
    % ---- 数据读取相关 ----
    cfg.data_var = '';     % 如果你知道 .mat 里的主数据变量名，可直接写，如 'data'
    cfg.fs = [];           % 留空 = 自动识别；若自动失败，请手动写采样率，如 500 或 1000
    cfg.verbose = true;

    % ---- 通道映射：按你的说明固定为 1/2/3/4 ----
    cfg.filtered_h_ch = 1; % 已滤波 水平眼电
    cfg.raw_h_ch      = 2; % 原始   水平眼电
    cfg.filtered_v_ch = 3; % 已滤波 垂直眼电
    cfg.raw_v_ch      = 4; % 原始   垂直眼电

    % ---- detect-blink 训练 / 事件阈值 ----
    cfg.train_secs       = 10;     % 前 10 秒用于无监督训练；数据太短时可改小
    ...
    % ---- 结果输出 ----
    cfg.save_qc_plot = true;  % 是否保存质量控制图
    cfg.save_debug_mat = true; % 是否保存每个文件的中间结果 mat
    cfg.xlsx_name = 'detect_blink_results.xlsx';
    ...
    for i = 1:numel(mat_files)
        ...
        try
            ...
            if cfg.save_debug_mat
                debug_result = result; %#ok<NASGU>
                debug_meta = meta; %#ok<NASGU>
                raw_signals = table(h_filt, h_raw, v_filt, v_raw); %#ok<NASGU>
...
function row = buildSummaryRow(result, file_name, fs, n_samples, duration_s, train_secs, meta, cfg, status_str, err_msg)
    ...
    if isstruct(result) && isfield(result, 'Nblinks')
        ...
        if isfield(result, 'model')
            row.model_mu_fix(1) = result.model.mu_fix;
            row.model_mu_bs(1)  = result.model.mu_bs;
            row.model_mu_sac(1) = result.model.mu_sac;
            row.model_mu_bli(1) = result.model.mu_bli;
        end
    end
end
