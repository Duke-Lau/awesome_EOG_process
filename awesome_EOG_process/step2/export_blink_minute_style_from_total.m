...
%       180, 5);
    ...
    if nargin < 1 || isempty(inputXlsx)
        inputXlsx = 'E:\26.04.09-灯具测试-EOG\mat-total\detect_blink_output\total_light_Blink.xlsx';
    end
    ...
    if nargin < 3 || isempty(train_secs)
        train_secs = 60;   % 前 60 s 为训练段，默认不纳入分钟统计
    end
    ...
    for i = 1:numel(files)
        ...
        [subjID, subjName, subjGroup] = parseSubjectName(thisFile);
        ...
        % 表头
        out(1,:) = {'被试ID', '姓名', '组别', 'BLI_DMM', '', 'BLI_DUR', '', 'BLI_PROB', 'BLI_TIME', ''};
        ...
        for s = 1:block_size_min:nFullMinutes
            ...
            out{s+1, 5}  = mean(minuteDMM(s:e)*1000,  'omitnan');
            out{s+1, 7}  = mean(minuteDUR(s:e),  'omitnan');
            out{s+1,10}  = mean(minuteCNT(s:e),  'omitnan');
        end

        % ---- 写出 ----
        outFile = fullfile(outputDir, sprintf('%s-%s-%s.xlsx', subjID, subjName ,subjGroup));
...
%% =========================================================
function [subjID, subjName ,subjGroup] = parseSubjectName(fileName)
